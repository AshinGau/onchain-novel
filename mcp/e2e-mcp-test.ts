...
import { bountyBoardAbi, novelCoreAbi, prizePoolAbi, rulesEngineAbi } from "./src/shared/abi.js";
import {
...
const prizePoolOwnerAbi = parseAbi(["function setKeeperRewardAmount(uint256 amount) external"]);
...
function makeClient(pk: string) {
  ...
  return {
    public: createPublicClient({ chain: foundry, transport: http(RPC_URL) }) as PublicClient<
      Transport,
      Chain
    >,
...
async function phase2CreateNovel() {
  ...
  const rootContent =
    "In the beginning there was nothing but the endless void. Then a spark ignited.";
  ...
  if (!createdLog || !createdLog.topics[1]) {
    ...
    info(
      `Expected sig: ${encodeEventTopics({ abi: novelCoreAbi, eventName: "NovelCreated" as any })[0]?.slice(0, 18)}`,
    );
    return;
  }
  ...
  if (novel.id !== novelId) {
    fail("getNovel: wrong ID");
    return;
  }
  if (novel.creator.toLowerCase() !== creator.address.toLowerCase()) {
    fail("getNovel: wrong creator");
    return;
  }
  if (novel.active !== true) {
    fail("getNovel: not active");
    return;
  }
  if (novel.currentRound !== 0) {
    fail("getNovel: round should be 0");
    return;
  }
  if (novel.roundPhase !== 0) {
    fail("getNovel: phase should be Idle(0)");
    return;
  }
  pass(
    `getNovel: verified (creator=${novel.creator.slice(0, 10)}..., active=${novel.active}, round=${novel.currentRound}, phase=${novel.roundPhase})`,
  );
  ...
  if (ancestors.length === 0) {
    fail("getWorldLineAncestors: empty after creation");
    return;
  }
  ...
  if (rootCh.novelId !== novelId) {
    fail("getChapter(root): wrong novelId");
    return;
  }
  if (rootCh.depth !== 1) {
    fail(`getChapter(root): depth should be 1, got ${rootCh.depth}`);
    return;
  }
  pass(
    `getChapter(root): #${rootCh.id}, depth=${rootCh.depth}, author=${rootCh.author.slice(0, 10)}...`,
  );
...
async function phase3SubmitChapters() {
  ...
  const contentA1 =
    "The spark grew into a flame that illuminated a vast cavern of crystal. Writer A explores the depths.";
  const contentB1 =
    "The spark drifted upward, becoming a star in an infinite sky. Writer B takes a different path entirely.";
  const contentA2 =
    "Deep within the crystal cavern, ancient runes began to glow. Writer A continues the underground journey.";
  ...
  if (ch.novelId !== novelId) {
    fail("getChapter(A1): wrong novelId");
    return;
  }
  if (ch.parentId !== rootChapterId) {
    fail("getChapter(A1): wrong parentId");
    return;
  }
  if (ch.depth !== 2) {
    fail(`getChapter(A1): depth should be 2, got ${ch.depth}`);
    return;
  }
...
async function phase4VotingRound() {
  ...
  if (novel.roundPhase !== 1) {
    fail(`After startRound: phase should be 1 (Nominating), got ${novel.roundPhase}`);
    return;
  }
  if (novel.currentRound !== 1) {
    fail(`After startRound: round should be 1, got ${novel.currentRound}`);
    return;
  }
  ...
  if (novel.roundPhase !== 2) {
    fail(`After closeNomination: phase should be 2 (Committing), got ${novel.roundPhase}`);
    return;
  }
  ...
  if (novel.roundPhase !== 3) {
    fail(`After closeCommit: phase should be 3 (Revealing), got ${novel.roundPhase}`);
    return;
  }
  ...
  if (novel.roundPhase !== 0) {
    fail(`After settleRound: phase should be 0 (Idle), got ${novel.roundPhase}`);
    return;
  }
...
async function phase7ApiTests() {
  ...
  try {
    ...
    if (!Array.isArray(data.novels)) {
      fail("API novels list: not array");
    } else if (data.novels.length === 0) {
      fail("API novels list: empty");
    } else {
      pass(`API: GET /api/novels (${data.novels.length} novels, total=${data.pagination?.total})`);
    }
  } catch (e: any) {
  ...
  try {
    const novel = await apiFetch<any>(`/api/novels/${novelId}`);
    if (!novel.id) {
      fail("API novel detail: no id");
    } else if (!novel.title) {
      fail("API novel detail: no title");
    } else {
      pass(`API: GET /api/novels/${novelId} — "${novel.title}" (chapters=${novel.chapter_count})`);
    }
  } catch (e: any) {
  ...
  try {
    ...
    if (!Array.isArray(data.chapters)) {
      fail("API tree: no chapters array");
    } else {
      pass(`API: GET /api/novels/${novelId}/tree (${data.chapters.length} chapters)`);
    }
  } catch (e: any) {
    ...
    try {
      ...
      if (!Array.isArray(data.chapters)) {
        fail("API chapters: no chapters array");
      } else {
        pass(`API: GET /api/novels/${novelId}/chapters (${data.chapters.length} chapters)`);
      }
    } catch (e2: any) {
  ...
  try {
    ...
    if (!Array.isArray(data.worldlines)) {
      fail("API worldlines: not array");
    } else {
      pass(`API: GET /api/novels/${novelId}/worldlines (${data.worldlines.length} lines)`);
    }
  } catch (e: any) {
  ...
  try {
    ...
    if (!ch.id) {
      fail("API chapter detail: no id");
    } else {
      const hasContent = "content_text" in ch;
      pass(
        `API: GET /api/chapters/${chapterA1Id} — author=${(ch.author as string)?.slice(0, 10)}... content=${hasContent ? (ch.content_text ? ch.content_text.length + " chars" : "null") : "no field"}`,
      );
    }
  } catch (e: any) {
  ...
  try {
    ...
    if (!Array.isArray(data.ancestors)) {
      fail("API context: not array");
    } else if (data.ancestors.length === 0) {
      fail("API context: empty");
    } else {
      pass(`API: GET /api/chapters/${chapterA2Id}/context (${data.ancestors.length} ancestors)`);
    }
  } catch (e: any) {
  ...
  try {
    ...
    if (!Array.isArray(data.chapters)) {
      fail("API user chapters: not array");
    } else {
      pass(
        `API: GET /api/users/${writerA.address.slice(0, 10)}.../chapters (${data.chapters.length} chapters)`,
      );
    }
  } catch (e: any) {
  ...
  try {
    ...
    if (!Array.isArray(data.votes)) {
      fail("API user votes: not array");
    } else {
      pass(
        `API: GET /api/users/${voterA.address.slice(0, 10)}.../votes (${data.votes.length} votes)`,
      );
    }
  } catch (e: any) {
