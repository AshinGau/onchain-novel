...
function parseNovelConfig(p: Record<string, unknown>): NovelConfig {
  return {
    ...
    maxVoterReward: parseEther((p.maxVoterReward as string) ?? "0"),
    unrevealPenaltyFloor: parseEther((p.unrevealPenaltyFloor as string) ?? "0.001"),
...
export function registerNovelTools(server: McpServer): void {
  ...
  server.tool(
    ...
    "Create a new collaborative novel on-chain with a root chapter.",
    {
      ...
      minChapterLength: z.number().default(100).describe("Min chapter bytes"),
      ...
      voteStake: z.string().default("0.005").describe("Vote stake in ETH"),
      nominationFee: z.string().default("0.01").describe("Nomination fee in ETH"),
      ...
      voterRewardRate: z.number().default(1500).describe("Voter reward basis points"),
      maxVoterReward: z.string().default("0").describe("Per-address voter reward cap per round in ETH (0 = uncapped)"),
      unrevealPenaltyFloor: z.string().default("0.001").describe("Minimum penalty for unrevealed votes in ETH"),
  ...
  server.tool(
    ...
    "Fork a novel from an existing chapter.",
    {
      ...
      voterRewardRate: z.number().default(1500),
      maxVoterReward: z.string().default("0"),
      unrevealPenaltyFloor: z.string().default("0.001"),
