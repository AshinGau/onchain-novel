#!/usr/bin/env bash
# =============================================================================
# Seed the running local chain with sample novels. Assumes scripts/dev.sh start
# has already deployed the contracts (addresses come from config.yaml).
#
...
# #1 creator, #2 writer A, #3 writer B, #6 writer C, #4 voter A, #5 voter B.
...
PK_WRITER_C="0x92db14e403b83dfe3df233f83dfa3a0d7096f21ca9b0d6d6b8d88b2b4ec1564e"
...
# Deploy.s.sol sets keeper = deployer (anvil account #0). Use that same key
# here for keeper-only phase transitions; any other key reverts NotKeeperYet.
PK_KEEPER="0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80"

# ── Helpers ──
# cast defaults to a 60s receipt timeout; anvil with --block-time 1 occasionally
# stalls for longer under seed load (state file grows, scheduler jitter). Bump
# to 180s so we don't false-negative on transient hiccups.
CAST_TIMEOUT=180

cast_send() {
    local pk="$1"; shift
    local result
    result=$(cast send --rpc-url "$RPC" --private-key "$pk" --timeout "$CAST_TIMEOUT" "$@" --json 2>&1) \
...
cast_send_value() {
    ...
    local result
    result=$(cast send --rpc-url "$RPC" --private-key "$pk" --timeout "$CAST_TIMEOUT" --value "$value" "$@" --json 2>&1) \
        || { echo "$result" >&2; die "cast send failed: $*"; }
    local status
    status=$(echo "$result" | jq -r '.status' 2>/dev/null || echo "")
    [ "$status" = "0x1" ] || { echo "$result" >&2; die "tx reverted: $*"; }
...
chapter_count() {
    cast call --rpc-url "$RPC" "$NOVEL_CORE" "chapterCount()(uint64)" 2>/dev/null \
        | awk '{print $1}'
}

novel_count() {
    cast call --rpc-url "$RPC" "$NOVEL_CORE" "novelCount()(uint64)" 2>/dev/null \
        | awk '{print $1}'
}
...
submit_chapter() {
    ...
    make_content "$text"
    cast_send_value "$pk" "0.01ether" "$NOVEL_CORE" \
...
run_round() {
    ...
    cast_send "$PK_KEEPER" "$ROUND_MANAGER" "startRound(uint64,uint64[])" "$novel_id" "$leaves"
    advance 6
    cast_send "$PK_KEEPER" "$ROUND_MANAGER" "closeNomination(uint64)" "$novel_id"
    ...
    cast_send_value "$PK_VOTER_A" "0.001ether" "$ROUND_MANAGER" \
        "commitVote(uint64,bytes32)" "$novel_id" "$commit_a"
    cast_send_value "$PK_VOTER_B" "0.001ether" "$ROUND_MANAGER" \
        "commitVote(uint64,bytes32)" "$novel_id" "$commit_b"

    advance 6
    cast_send "$PK_KEEPER" "$ROUND_MANAGER" "closeCommit(uint64)" "$novel_id"
    ...
    cast_send "$PK_KEEPER" "$ROUND_MANAGER" "settleRound(uint64)" "$novel_id"
...
cast_send_value "$PK_CREATOR" "0.5ether" "$NOVEL_CORE" \
...
cast_send_value "$PK_CREATOR" "0.3ether" "$NOVEL_CORE" \
...
cast_send_value "$PK_VOTER_B" "0.5ether" "$BOUNTY_BOARD" \
...
cast_send_value "$PK_VOTER_A" "0.2ether" "$BOUNTY_BOARD" \
...
cast_send_value "$PK_VOTER_B" "0.3ether" "$BOUNTY_BOARD" \
...
# Note: designateBounty requires child.timestamp ∈ [bounty.createTime, deadline]
# AND child.parentId == bounty target. Existing chapters like N3_C8 were
# submitted long before this bounty, so they fail the timestamp check. We
# create a fresh child of N3_C5 *after* funding the bounty, then designate it.
cast_send_value "$PK_VOTER_A" "0.1ether" "$BOUNTY_BOARD" \
    "createBounty(uint64,uint64)" "$N3_C5" "$BOUNTY_DEADLINE"
submit_chapter "$PK_WRITER_A" "$NOVEL3_ID" "$N3_C5" \
    "Drawn by the bounty, a new voice answered the archivist. The pre-genesis process unfurled another layer, and this time someone was there to witness its reveal in real time."
N3_BOUNTY_CHILD=$CHAPTER_ID
cast_send "$PK_VOTER_A" "$BOUNTY_BOARD" \
    "designateBounty(uint64,uint64)" "3" "$N3_BOUNTY_CHILD"
ok "Bounty: 0.1 ETH on Novel 3 ch#$N3_C5, designated ch#$N3_BOUNTY_CHILD"
...
set_nickname() {
    ...
    cast_send "$pk" "$USER_REGISTRY" "setNickname(bytes32)" "$hex"
}
