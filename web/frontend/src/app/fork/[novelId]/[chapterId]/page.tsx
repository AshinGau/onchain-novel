...
export default function ForkNovelPage({
    ...
    async function load() {
      try {
        ...
        setConfig({
          ...
          maxVoterReward: c.maxVoterReward ? (Number(c.maxVoterReward) / 1e18).toString() : "0",
          unrevealPenaltyFloor: c.unrevealPenaltyFloor ? (Number(c.unrevealPenaltyFloor) / 1e18).toString() : "0",
  ...
  if (loading) {
    return <div className="on-container" style={{ maxWidth: "800px" }}><p className="text-caption">Loading source novel...</p></div>;
  }
  if (fetchError) {
    return <div className="on-container" style={{ maxWidth: "800px" }}><p className="text-danger">{fetchError}</p></div>;
  }

  return (
    <div className="on-container" style={{ maxWidth: "800px" }}>
      <Link href={`/novels/${novelId}`} className="text-link text-caption">← Back to Novel</Link>
      <h1 className="text-heading" style={{ marginTop: "1rem" }}>Fork a Novel</h1>
      <p className="text-caption" style={{ marginBottom: "2rem" }}>Create a new novel branching from an existing chapter.</p>

      <div className="on-card" style={{ marginBottom: "1.5rem" }}>
        <h2 className="text-subheading">Fork Source</h2>
        <p className="text-caption">
          Forking from <strong>{sourceNovel?.title || `Novel #${novelId}`}</strong>, Chapter #{chapterId}
        </p>
      ...
      <form onSubmit={handleSubmit} className="on-stack on-stack-lg">
        ...
        <div className="on-card on-stack">
          ...
          <div>
            ...
            <textarea value={description} onChange={(e) => setDescription(e.target.value)}
              placeholder="Describe your forked novel..." rows={3} className="on-form-textarea" />
          </div>
          <div>
        ...
        {tx.status === "success" && (
          <div className="on-card" style={{ borderColor: "var(--color-success)", color: "var(--color-success)" }}>Novel forked! Redirecting...</div>
        )}

        <button type="submit" disabled={tx.isPending || !isConnected}
