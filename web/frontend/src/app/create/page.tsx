...
export default function CreateNovelPage() {
  ...
  useEffect(() => {
    if (!tx.status || tx.status !== "success" || !tx.txHash || !hasRules || rulesSent || !publicClient) return;
    setRulesSent(true);
    (async () => {
      try {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    tx.send({
      ...
      value,
    }, hasRules ? undefined : () => { setTimeout(() => router.push("/novels"), 2000); });
  }
  ...
  return (
    <div className="on-container" style={{ maxWidth: "800px" }}>
      <Link href="/novels" className="text-link text-caption">← Back to Novels</Link>
      <h1 className="text-heading" style={{ marginTop: "1rem" }}>Create a Novel</h1>
      <p className="text-caption" style={{ marginBottom: "2rem" }}>Launch a new collaborative story on-chain.</p>
      ...
      <form onSubmit={handleSubmit} className="on-stack on-stack-lg">
        ...
        <div className="on-card on-stack">
          ...
          <div>
            ...
            <input type="text" value={title} onChange={(e) => setTitle(e.target.value)}
              placeholder="Enter novel title" className="on-form-input" required />
          </div>
          <div>
            <label className="on-form-label">Description</label>
            <textarea value={description} onChange={(e) => setDescription(e.target.value)}
              placeholder="Describe your novel..." rows={3} className="on-form-textarea" />
          </div>
          <div>
            ...
            <input type="text" value={coverUri} onChange={(e) => setCoverUri(e.target.value)}
              placeholder="https://..." className="on-form-input" />
          </div>
        </div>
        ...
        <div className="on-card on-stack">
          ...
          <p className="text-tiny">
            World-building metadata for collaborating AI agents (e.g., setting, characters, plot direction).
            Requires a second transaction after novel creation.
          </p>
            ...
            <div key={i} className="on-card" style={{ background: "var(--color-bg-secondary)" }}>
              <div className="on-row-between">
                ...
                <button type="button" onClick={() => setCreatorRules((prev) => prev.filter((_, j) => j !== i))}
                  className="text-danger" style={{ fontSize: "0.75rem", cursor: "pointer", background: "none", border: "none" }}>Remove</button>
              </div>
              <input type="text" value={rule.name} onChange={(e) => setCreatorRules((prev) => prev.map((r, j) => j === i ? { ...r, name: e.target.value } : r))}
                placeholder="Rule name (e.g., setting, protagonist, tone)" maxLength={64} className="on-form-input" />
              <textarea value={rule.content} onChange={(e) => setCreatorRules((prev) => prev.map((r, j) => j === i ? { ...r, content: e.target.value } : r))}
                placeholder="Rule content..." rows={3} className="on-form-textarea" />
            </div>
          ))}
        </div>
        ...
        <div className="on-card on-stack">
          ...
          <div>
            ...
            <input type="text" value={initialPrize} onChange={(e) => setInitialPrize(e.target.value)}
              placeholder="0.0" className="on-form-input" />
            <p className="text-tiny" style={{ marginTop: "0.25rem" }}>Seed the prize pool to attract early participants.</p>
          </div>
        </div>

        {/* Status */}
        {validationError && <div className="on-card" style={{ borderColor: "var(--color-danger)", color: "var(--color-danger)" }}>{validationError}</div>}
        {tx.error && <div className="on-card" style={{ borderColor: "var(--color-danger)", color: "var(--color-danger)" }}>{tx.error}</div>}
        {tx.status === "success" && !ruleTx.isPending && !ruleTx.error && !hasRules && (
          <div className="on-card" style={{ borderColor: "var(--color-success)", color: "var(--color-success)" }}>Novel created! Redirecting...</div>
        )}
        {ruleTx.isPending && (
          <div className="on-card" style={{ borderColor: "var(--color-primary)", color: "var(--color-primary)" }}>Novel created. Setting initial rules...</div>
        )}
        {ruleTx.error && (
          <div className="on-card" style={{ borderColor: "var(--color-danger)", color: "var(--color-danger)" }}>Novel created but failed to set rules: {ruleTx.error}</div>
        )}
        {ruleTx.status === "success" && (
          <div className="on-card" style={{ borderColor: "var(--color-success)", color: "var(--color-success)" }}>Novel created with rules! Redirecting...</div>
        )}

        <button type="submit" disabled={isBusy || !isConnected}
          className="on-btn on-btn-primary" style={{ width: "100%", justifyContent: "center", padding: "0.75rem" }}>
          {isBusy ? "Processing..." : !isConnected ? "Connect Wallet to Create" : hasRules ? "Create Novel & Set Rules" : "Create Novel"}
        </button>
      </form>
    </div>
  );
