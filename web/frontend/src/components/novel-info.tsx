...
import { formatBalance, formatDuration, shortAddress } from "@/lib/format";
...
function Tip({ text }: { text: string }) {
  ...
  return (
    <span
      ref={ref}
      style={{ position: "relative", display: "inline-block", marginLeft: "0.25rem" }}
    >
      <button
        type="button"
      ...
      {open && <span className="on-tip-popup">{text}</span>}
    </span>
  );
}
...
function usePhaseRemaining(novel: Novel): string | null {
  ...
  useEffect(() => {
    ...
    novel.active,
    novel.round_phase,
    novel.phase_start_time,
    novel.last_settle_time,
    novel.config,
  ]);
  ...
  return (
    <div className="on-card on-stack" style={{ gap: "0.75rem" }}>
      ...
      <div className="on-row" style={{ gap: "1rem", alignItems: "flex-start" }}>
        {novel.cover_uri && <img src={novel.cover_uri} alt="" className="on-cover on-cover-lg" />}
          ...
          {phaseRemaining && (
            <span
              style={{
                marginLeft: "0.25rem",
                color:
                  phaseRemaining === "changing" ? "var(--color-success)" : "var(--color-warning)",
              }}
            >
              ({phaseRemaining})
            </span>
      ...
      {novel.active && novel.round_phase === 0 && novel.current_round > 0 && worldLineCount > 0 && (
        ...
        >
          {worldlinesWithContinuations >= worldLineCount ? (
            <span>
              ✓ Ready for next round: all {totalWorldlines} world line(s) have continuations.
            </span>
          ) : (
            <span>
              Waiting for continuations:{" "}
              <b>
                {worldlinesWithContinuations}/{totalWorldlines}
              </b>{" "}
              world line(s) have a next chapter. The next round starts automatically when every
              world line has at least one continuation. Help out by writing the next chapter or
              sponsoring one.
            </span>
          )}
        </div>
        ...
        {configOpen && c && (
          ...
          >
            ...
            <ConfigItem
              ...
              value={["Onchain", "External (IPFS)", "HTTP"][c.contentLocation] || "Unknown"}
              tip="Where chapter content is stored. Onchain = in calldata; External = IPFS/Arweave; HTTP = CDN."
            />
            {c.ruleFee !== undefined && (
              <ConfigItem
                label="Rule Proposal Fee"
                value={fmtWei(c.ruleFee)}
            ...
            {c.ruleVoteDuration !== undefined && (
              <ConfigItem
                ...
                value={formatDuration(c.ruleVoteDuration)}
