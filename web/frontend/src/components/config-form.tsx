...
import { CONTENT_LOCATIONS, validateField, type NovelConfigForm } from "@/lib/novel-config";
...
export function ConfigForm({ config, onChange, contentLocationReadOnly }: ConfigFormProps) {
  ...
  const update = useCallback(
    <K extends keyof NovelConfigForm>(key: K, value: NovelConfigForm[K]) => {
    ...
    <div className="on-stack">
      ...
      <Section title="Chapter">
        <div className="on-grid on-grid-2">
          <NumberField
            label="Min Chapter Length (bytes)"
            tip="Minimum content size in bytes."
            value={config.minChapterLength}
            onChange={(v) => update("minChapterLength", v)}
            onBlur={() => blur("minChapterLength")}
            error={errors.minChapterLength}
            min={1}
          />
          <NumberField
            label="Max Chapter Length (bytes)"
            tip="Maximum content size in bytes."
            value={config.maxChapterLength}
            onChange={(v) => update("maxChapterLength", v)}
            onBlur={() => blur("maxChapterLength")}
            error={errors.maxChapterLength}
            min={1}
          />
        </div>
      </Section>
      ...
      <Section title="Rounds">
        <div className="on-grid on-grid-2">
          <NumberField
            label="World Line Count"
            tip="Parallel story branches kept each round."
            value={config.worldLineCount}
            onChange={(v) => update("worldLineCount", v)}
            onBlur={() => blur("worldLineCount")}
            error={errors.worldLineCount}
            min={1}
          />
          <DurationField
            label="Min Round Gap"
            tip="Minimum interval between rounds."
            value={config.minRoundGap}
            onChange={(v) => update("minRoundGap", v)}
            error={errors.minRoundGap}
          />
          <DurationField
            label="Nominate Duration"
            tip="Time for candidate nominations."
            value={config.nominateDuration}
            onChange={(v) => update("nominateDuration", v)}
            error={errors.nominateDuration}
          />
        </div>
      </Section>
      ...
      <Section title="Voting">
        <div className="on-grid on-grid-2">
          <DurationField
            ...
            tip="Time for voters to submit encrypted commitments."
            value={config.commitDuration}
            onChange={(v) => update("commitDuration", v)}
            error={errors.commitDuration}
          />
          <DurationField
            label="Reveal Duration"
            tip="Time for voters to reveal votes."
            value={config.revealDuration}
            onChange={(v) => update("revealDuration", v)}
            error={errors.revealDuration}
          />
        </div>
      </Section>
      ...
      <Section title="Economics">
        <div className="on-grid on-grid-2">
          ...
          <div>{/* spacer */}</div>
          <PercentSlider
            label="Prize Release Rate"
            tip="Percentage of prize pool released each round."
            value={config.prizeReleaseRate}
            onChange={(v) => update("prizeReleaseRate", v)}
          />
          <PercentSlider
            label="Voter Reward Rate"
            tip="Share of round rewards allocated to voters."
            value={config.voterRewardRate}
            onChange={(v) => update("voterRewardRate", v)}
            max={5000}
          />
        </div>
      </Section>
      ...
      <Section title="Rules (AI Agents)">
        <div className="on-grid on-grid-2">
          ...
          <NumberField
            label="Rule Vote Quorum"
            tip="World-line-author votes needed to pass."
            value={config.ruleQuorum}
            onChange={(v) => update("ruleQuorum", v)}
            onBlur={() => blur("ruleQuorum")}
            error={errors.ruleQuorum}
            min={0}
          />
          <DurationField
            label="Rule Vote Duration"
            tip="Time window for rule proposal voting."
            value={config.ruleVoteDuration}
            onChange={(v) => update("ruleVoteDuration", v)}
            error={errors.ruleVoteDuration}
          />
        </div>
      </Section>
        ...
        <div>
          ...
          {contentLocationReadOnly ? (
            <p className="text-caption" style={{ marginTop: "0.25rem" }}>
              {selectedLoc?.label} — {selectedLoc?.desc}
            </p>
          ) : (
            <>
              <div className="on-row" style={{ marginTop: "0.25rem" }}>
                {CONTENT_LOCATIONS.map((loc) => (
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      update("contentLocation", loc.value);
                      blur("contentBaseUrl");
                    }}
