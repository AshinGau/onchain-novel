...
1. **Phase 1 - 合约核心**：DataTypes + NovelCore（章节双向树 + DFS 候选生成 + round 生命周期）+ PrizePool（含 tipNovel / tipChapter）
2. **Phase 2 - 投票引擎**：VotingEngine（三阶段投票 + 简化奖励）
3. **Phase 3 - 规则引擎**：RulesEngine 适配
4. **Phase 4 - 续写悬赏**：BountyBoard（独立模块）
5. **Phase 5 - 后端**：Indexer 适配 + API 调整
6. **Phase 6 - MCP**：工具接口更新
7. **Phase 7 - 前端**：UI 重构
